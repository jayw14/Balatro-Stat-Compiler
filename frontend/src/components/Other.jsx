...
function Other(props){
    ...
    sortedList.forEach((e) =>{
        ...
        if(props.item != "hands"){
            let img = new Image()
            img.src = `src/images/consumables/${e.name}.webp`
        }
    ...
    if(props.item == "hands"){
        ...
        return pageList.map((e, index) => {
            ...
            return <div key={index} className="singleDisplay">
                <img src={`src/images/vouchers/${name}.webp`} alt={`Image of ${name}`} />
        ...
        return pageList.map((e, index) => {
            ...
            return <div key={index} className="singleDisplay">
                <img src={`src/images/consumables/${name}.webp`} alt={`Image of ${name}`}/>
                <p className={props.item}>{pageList[index].count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
